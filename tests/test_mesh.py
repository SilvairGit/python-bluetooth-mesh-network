...
from bluetooth_mesh.network.mesh import (
    AccessMessage,
    ControlMessage,
    MeshPrivateBeacon,
    NetworkMessage,
...
@fixture(name="health_current_status_message")
def fixture_health_current_status_message():
    payload = bytes.fromhex("0400000000")
    return AccessMessage(src=0x1201, dst=0xFFFF, ttl=0x03, payload=payload)


@fixture(name="config_appkey_status_message")
def fixture_config_appkey_status_message():
    payload = bytes.fromhex("800300563412")
    return AccessMessage(src=0x1201, dst=0x0003, ttl=0x0B, payload=payload)


@fixture(name="config_appkey_add_message")
def fixture_config_appkey_add_message():
...
def test_private_beacon_received_iv_update(net_key):
    ...
    beacon, _auth = MeshPrivateBeacon.unpack(
        bytes.fromhex("435f18f85cf78a3121f58478a561e488e7cbf3174f022a514741"), private_beacon_key
    )
...
def test_private_beacon_received_iv_update_complete(net_key):
    ...
    private_beacon_key = net_key.private_beacon_key
    beacon, _auth = MeshPrivateBeacon.unpack(
        bytes.fromhex("1b998f82927535ea6f3076f422ce827408ab2f0ffb94cf97f881"), private_beacon_key
    )
...
def test_unprovisioned_beacon_created():
    beacon = UnprovisionedDeviceBeacon(uuid.UUID("25bdf2eb-03cc-4383-a65a-dd3e8007fb55"), 0x4243)
...
def test_network_nonce(config_appkey_status_message):
    assert Nonce(
        ...
        False,
    ).network(seq=0x000006, iv_index=0x12345678) == bytes.fromhex("000b0000061201000012345678")
...
def test_device_nonce(config_appkey_status_message):
    assert Nonce(
        ...
        config_appkey_status_message.ttl,
        False,
    ).device(seq=0x000006, iv_index=0x12345678) == bytes.fromhex("02000000061201000312345678")
...
def test_application_nonce(health_current_status_message):
    assert Nonce(
        ...
        False,
    ).application(seq=0x000007, iv_index=0x12345678) == bytes.fromhex("01000000071201ffff12345678")


def test_device_pdu(config_appkey_status_message, dev_key):
    (segment,) = config_appkey_status_message.segments(dev_key, seq=0x000006, iv_index=0x12345678)

    assert segment == bytes.fromhex("0089511bf1d1a81c11dcef")


def test_application_pdu(health_current_status_message, app_key):
    (segment,) = health_current_status_message.segments(app_key, seq=0x000007, iv_index=0x12345678)

    assert segment == bytes.fromhex("665a8bde6d9106ea078a")


def test_application_pdu_segmented(config_appkey_add_message, dev_key):
    segments = list(config_appkey_add_message.segments(dev_key, seq=0x3129AB, iv_index=0x12345678))
...
def test_application_pdu_segmented_long_mic(dev_key):
    ...
    segments = list(bogus_long_message.segments(dev_key, seq=0x3129AB, iv_index=0x12345678))
...
def test_application_pack_to_network_pdu(
    ...
    _seq, network_pdu = next(network_message.pack(app_key, net_key, seq=0x000007, iv_index=0x12345678))
...
def test_application_pack_to_network_pdu_skip_segments(
    ...
    _seq, network_pdu = next(
        network_message.pack(
            app_key,
            net_key,
            seq=0x000007,
            iv_index=0x12345678,
...
def test_application_network_pdu_segmented_retry(health_current_status_message, app_key, net_key):
    ...
    _seq, network_pdu = next(
        network_message.pack(app_key, net_key, seq=0x000017, iv_index=0x12345678, transport_seq=0x000007)
    )

    assert _seq == 0x000017
...
def test_control_segment_ack_message(control_appkey_add_ack_message, app_key, net_key):
    ...
    _seq, network_pdu = next(network_message.pack(app_key, net_key, seq=0x014835, iv_index=0x12345678))
...
def test_control_pack_to_network_pdu(control_friend_offer_message, app_key, net_key):
    ...
    _seq, network_pdu = next(network_message.pack(app_key, net_key, seq=0x014820, iv_index=0x12345678))

    assert network_pdu.hex() == "68d4c826296d7979d7dbc0c9b4d43eebec129d20a620d01e"


def test_control_unpack_from_network_pdu(
    control_friend_offer_message: ControlMessage, app_key: ApplicationKey, net_key: NetworkKey
):
    _, _, unpacked_network_message = NetworkMessage.unpack(
        app_key,
        net_key,
...
def test_proxy_config_pack_to_network_pdu(
    ...
    _seq, network_pdu = next(network_message.pack(app_key, net_key_local, seq=0x000001, iv_index=0x12345678))
...
def test_proxy_solicitation_pack_to_network_pdu(
    ...
    _seq, network_pdu = next(network_message.pack(app_key, net_key_local, seq=0x000001, iv_index=0x00000000))
