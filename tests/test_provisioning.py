...
from bluetooth_mesh.network.mesh import ProvisioningTransaction
from bluetooth_mesh.network.provisioning import (
    BearerOpcode,
...
valid = [
    ...
    pytest.param(
        bytes.fromhex("010100030101080009050002"),
        {
            ...
            "parameters": {
                "num_elements": 1,
                "algorithms": {
                    ProvisioningAlgorithm.P256_CMAC_AES128,
                    ProvisioningAlgorithm.P256_HMAC_SHA256,
                },
                "public_key_type": {ProvisioningPublicKeyType.OOB},
                "static_oob_type": {ProvisioningStaticOOBType.AVAILABLE},
                "output_oob_size": 8,
                "output_oob_action": {
                    ProvisioningOutputOOBAction.BLINK,
    ...
    pytest.param(
        bytes.fromhex("07d0bd7f4a89a2ff6222af59a90a60ad58acfe3123356f5cec2973e0ec50783b10c7"),
        {
...
def test_confirmation():
    ...
    ecdh_secret = bytes.fromhex("ab85843a2f6d883f62e5684b38e307335fe6e1945ecd19604105c6f23221eb69")
    random = bytes.fromhex("8b19ac31d58b124c946209b5db1021b9")

    auth = None

    _salt, _key, confirmation = ProvisioningEncryption.confirmation_encrypt(
        ecdh_secret,
...
bearer_ctrl = [
    pytest.param(
        bytes.fromhex("03 70cf7c9732a345b691494810d2e9cbf4"),
        {
            ...
            "parameters": {"device_uuid": bytes.fromhex("70cf7c9732a345b691494810d2e9cbf4")},
    ...
    pytest.param(
        bytes.fromhex("0b00"),
        {
            ...
            "parameters": {"reason": LinkCloseReason.SUCCESS},
            "gpcf": GenericProvisioningPDUType.CONTROL,
        },
        id="link close",
    ),
...
def test_encrypt_provisioning_data(params):
    prov_data = (
        params["net_key"] + params["net_key_index"] + params["flags"] + params["ivindex"] + params["address"]
    )
